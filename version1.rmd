...
data.frame(Missings = sapply(ucr, function(x) sum(is.na(x))))
```
...
```{r, fig.width = 8, fig.height=3, message=FALSE, warning=FALSE, fig.align='center'}
...
#### Show data
```{r}
rape_df %>% rename(Year = year, 
                   `Rape Revised` = rape_revised_count,
                   `Rape Legacy` = rape_legacy_count)
```
...
pl[[1]][["labels"]][["title"]] <- "Violent crimes (total)"
pl[[2]][["labels"]][["title"]] <- "Murder and manslaughter"
pl[[3]][["labels"]][["title"]] <- "Robbery"
pl[[4]][["labels"]][["title"]] <- "Aggravate assault"
pl[[5]][["labels"]][["title"]] <- "Property crimes (total)"
pl[[6]][["labels"]][["title"]] <- "Burglary"
pl[[7]][["labels"]][["title"]] <- "Larceny"
pl[[8]][["labels"]][["title"]] <- "Vehicle theft"
...
prison %>% rename(Jurisdiction = jurisdiction,
                  Year = year,
                  `If includes jails` = includes_jails,
                  `Number of prisoners` = prison) %>%
  head() %>% 
  kable_f()
```
...
prison_exp %>% rename(Year = year,
                      Jurisdiction = jurisdiction,
                      `Prison expenditures` = prison_expenditure) %>%
  head() %>% 
  kable_f()
```
...
us_states_info %>% rename(Jurisdiction = jurisdiction,
                          Region = region,
                          `Area (km^2)` = area_km2) %>%
  head() %>% 
  kable_f()
...
The district of Columbia is named as a jurisdiction and a separate row in the prison_exp and ucr datasets, however they do not appear in prison dataset.
...
colnames(dc.df) = c("Median values for other states", "Values for District of Columbia", "DC/avg states in %")
kable_f(dc.df)
```
...
```{r, fig.height = 3, fig.width = 8, warning=FALSE, message=FALSE, fig.align='center'}
plot.data <- ucr %>% 
...
ggp1 <- ggplot(data = plot.data, aes(x=region, y=value, fill=variable)) +
   ...
   scale_fill_viridis_d(name="Type of crime",
                        labels = c("Violent", "Property"),
                        begin = 0.3,
                        end=0.8) +
  labs(title = "Proportion of crimes per state area",  y = "Share percentage")
...
violent_plot <- plot_ly(violent, color = I("gray80"),
  ...
  layout(title="Percent change in state violent crime rates, 2001 vs 2016",
         ...
         autosize = F,
         height = 1000)
property_plot <- plot_ly(property, color = I("gray80"),
        text = ~paste('Pct change: ', round(pct_change_property), '%')) %>%
  ...
  layout(title="Percent change in state property crime rates, 2001 vs 2016",
         ...
         autosize = F,
         height = 1000)
imprisonment_plot <- plot_ly(imprisonment, color = I("gray80"),
        text = ~paste('Pct change: ', round(pct_change_imprisonment), '%')) %>%
...
```{r warning=FALSE, message=FALSE, fig.height=11, fig.width=6, fig.align='center'}
...
<img id="image_canv" src="xdensity.png" width="500">
<img id="image_canv" src="blankPlot.png" width="200">
<img id="image_canv" src="scatterPlot.gif" width="500">
<img id="image_canv" src="ydensity.png"  width = "200">
<img id="image_canv" src="legends.png">
...
Below can be seen a table regarding mapping jurisdiction and its cluster to different regions. 
...
# Conclusions
This paper presents the analysis of crime and incarceration in the United States using information on the level of different types of crimes, number of prisoners and federal expenditures on corrections. It turns out that there are on average 10 times more property crimes than violent crimes per square km. Additionally the Northeastern region has the highest number of crimes compared to other regions. Some Midwestern states like New York, Pennsylvania, New Jersey and Massachusets are ranked higher with violent crimes than property crimes compared to other states. The results of the analysis of the relationships between the number of prisoners per capita and the rates of violent crimes per capita showed a positive relationship between these variables as predicted. Over time, the relationship is ambiguous since some states show a negative and some positive relation in time. Additionally for both violent and property crimes, the numbers drop after 2009, an effect that can be especially noticed with property crimes. The highest swings between values in crimes in 2001 and 2016 can be noticed for North and South Dakotas, mainly in 2005 and 2006 when the crime rates spiked. As for incarceration rates, the state with the highest percent change of incarceration rates is West Virginia. We couldn't unequivocally define the relationship between investments into prisons and crime rates. Finally we performed clustering analysis on states based on different types of crimes resulting in 3 clusters that could be approximattely matched with the geographical locations of the states.
