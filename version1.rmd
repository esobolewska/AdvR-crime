...
```{r warning=FALSE, message=FALSE}
data_to_cluster <- ucr %>% dplyr::select(-c(crime_reporting_change, crimes_estimated, violent_crime_total, property_crime_total)) %>% filter(year==2017 & jurisdiction!="District of Columbia")
...
data_to_cluster <- data_to_cluster %>% select(-c(year, jurisdiction, state_population, murder_manslaughter, robbery, agg_assault, burglary, larceny, vehicle_theft))

data_to_cluster <- scale(data_to_cluster)
```

Below we can see visualised dissimilarity matrix, which show distant observations in yellow and close ones in blue We can easily notice groups of states with similar trends in criminality and also those which differentiate the most. The analysis was conducted based on 
...
heatmaply(distance,  
          ...
          main = 'Dissimilarity matrix of states')
```

According to the result of dissimilarity matrix, we decided to cluster states hierarchically, assuming three clusters. The algorithm 
