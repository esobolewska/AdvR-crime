...
The second dependency we want to reveal is the impact of investment on imprisonment on public safety quantified by the number of different kinds of crimes. 
...
```{r}
data_to_cluster_1 <- ucr %>% dplyr::select(-c(crime_reporting_change, crimes_estimated, violent_crime_total, property_crime_total)) %>% filter(year==2017 & jurisdiction!="District of Columbia")
...
fviz_dist(distance, 
          lab_size = 6.5,
          gradient = list(low = viridis(3)[1], mid = viridis(3)[2], high = viridis(3)[3]))
```
