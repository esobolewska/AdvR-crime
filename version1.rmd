...
```{r}
ucr_others <- ucr[,c(1:4)]
ucr_numeric <- ucr[,c(5:21)]

ucr_numeric <- sapply(ucr_numeric, function(x) format(x, nsmall=0, big.mark=","))
ucr <- cbind(ucr_others, ucr_numeric)
```
...
```{r}
prison_others <- prison[,c(1:2)]
prison_numeric <- prison[,c(3:18)]

prison_numeric <- sapply(prison_numeric, function(x) format(x, nsmall=0, big.mark=","))
prison <- cbind(prison_others, prison_numeric)
```
...
```{r}
prison_panel <- as.data.frame(prison_panel)

prison_panel_others <- prison_panel[,c(1:3)]
prison_panel_numeric <- prison_panel[,4]

prison_panel_numeric <- sapply(prison_panel_numeric, function(x) format(x, nsmall=0, big.mark=","))
prison_panel <- cbind(prison_panel_others, prison_panel_numeric)
```
...
```{r}
prison_exp_2016_others <- prison_exp_2016[,1]
prison_exp_2016_numeric <- prison_exp_2016[,2]

prison_exp_2016_numeric <- sapply(prison_exp_2016_numeric, function(x) format(x, nsmall=0, big.mark=","))
prison_exp_2016 <- cbind(prison_exp_2016_others, prison_exp_2016_numeric)
```
...
```{r warning=FALSE}
us_states_info_others <- us_states_info[,c(1:2)]
us_states_info_numeric <- us_states_info[,3]

us_states_info_numeric <- sapply(us_states_info_numeric, function(x) format(x, nsmall=0, big.mark=","))
us_states_info <- cbind(us_states_info_others, us_states_info_numeric)
```
...
```{r warning=FALSE}
df <- prison_panel %>% group_by(year) %>% summarise(value = sum(prison))
head(df)
```

```{r warning=FALSE}
p <- ggplot(data = df, aes(x = as.factor(year), y = value, color = year, group = 1,
...
ggp1 <- ggplot(data = us_states_ucr) +
    ...
    theme_minimal() +
    labs(title = "Property crimes per population")

ggp2 <- ggplot(data = us_states_ucr) +
    geom_sf(aes(fill = violent_crime_per_pop), lwd = 0) +
    scale_fill_viridis_c(option = "viridis", trans = "sqrt") +
    theme_minimal() +
    labs(title = "Violent crimes per population")
```
